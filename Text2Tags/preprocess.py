...
def raw_to_public(data_path: str, outpath: str, force: bool=False):
    ...
    df = df.loc[:, ['classification_id', 'user_id', 'workflow_id', 'annotations', 'subject_ids']]
    ...
    if "expert" in outpath:
        ...
        df.loc[:, 'annotations'] = df.annotations.str.join(",")
    df.to_csv(outpath)
    return

def case_correction(df: pd.DataFrame) -> pd.DataFrame:
    """Make all annotations lower case."""
    df.loc[:, "annotations"] = df.annotations.str.lower()
    return df

def remove_accents(df: pd.DataFrame) -> pd.DataFrame:
    df.loc[:, "annotations"] = df.annotations.apply(lambda x: unidecode.unidecode(str(x)))
    return df

def edge_case_slashes(df: pd.DataFrame) -> pd.DataFrame:
    """Replace slashes with commas as appropriate for the uncleaned data."""
    df.loc[:, "annotations"] = df.annotations.str.replace('/', ',')
    return df

def edge_case_periods(df: pd.DataFrame) -> pd.DataFrame:
    """Handle periods as a replacement for commas in the annotations."""
    df.loc[:, "annotations"] = df.annotations.str.strip('.')
    df.loc[:, "annotations"] = df.annotations.str.replace("3.4", "", regex=False)
    df.loc[:, "annotations"] = df.annotations.str.replace("host on. the image edge", "host on the image edge", regex=False)
    df.loc[:, "annotations"] = df.annotations.str.replace(".", ",", regex=False)
    return df
...
def edge_case_ampersands(df:pd.DataFrame) -> pd.DataFrame:
    """Process ampersands within annotations."""
    ...
    df.loc[df.annotations.str.contains("&\S", regex=True, na=False)].annotations.replace(" &", " and ", regex=False, inplace=True)
    df.loc[:, "annotations"] = df.annotations.str.replace("&", "and", regex=False)
    #print(df[df.annotations.str.contains("&", na=False)])
    return df
...
def explode_annotations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[:, "annotations"] = df.annotations.str.split(pat=',')
    df = df.explode("annotations", ignore_index=True)
    if "original_annotations" not in df.columns:
        df["original_annotations"] = df["annotations"].copy()
    return df

def manual_corrections(tag: str) -> str:
    manual_corrections_ = {
        # Corrections determined through visual inspection of data.
        "outofframe": "out of frame",
        "sshape": "s shape", "xshape": "x shape",
        "headtail": "head tail",
        "overedge": "over edge",
        "galaxyoverlap": "galaxy overlap",
        "doublehourglass": "double hourglass",
        "triplehourglass": "triple hourglass",
        "overedge": "over edge",
    ...
    for key in manual_corrections_.keys():
        if key in tag:
            tag = manual_corrections_[key]
    return tag
...
def edge_case_manual_corrections(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[:, "annotations"] = df.annotations.apply(manual_corrections)
    return df
...
def edge_case_host_references(df: pd.DataFrame, references: list[str] = ["optical", "dss", "wise"]) -> pd.DataFrame:
...
def expand_contractions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[:, "annotations"] = df.annotations.apply(lambda x: contractions.fix(str(x)))
    return df

def lemmatization(df: pd.DataFrame) -> pd.DataFrame:
    """Lemmatize strings."""
    raise NotImplementedcError
...
def apply_stopwords(
    ...
    df.loc[:, "annotations"] = df.annotations.str.split().apply(remove_stop_words, stopwords=stopwords)
    df.dropna()
    return df
...
def main():
    ...
    # Explode now appearing annotations
    df = explode_annotations(df)
